...
function DownloadLink({ href, children }: { href: string; children: ReactNode }) {
  return (
    <a
      href={href}
      download
      className="focus-ring inline-flex items-center gap-2 rounded-[10px] border border-line bg-paper px-3.5 py-2.5 text-xs font-semibold transition-colors hover:border-ink/25 hover:bg-white"
    >
...
function ThresholdVisual() {
  return (
    <div className="font-mono" aria-hidden>
      <p className="text-[11px] uppercase tracking-[0.1em] text-muted">significant</p>
    ...
    <TooltipProvider delayDuration={250}>
      <section className="mx-auto max-w-7xl pb-20 pt-12 sm:pt-16">
        <header className="grid items-end gap-7 lg:grid-cols-[minmax(0,1.2fr)_minmax(20rem,0.8fr)]">
          <div>
            <h1 className="max-w-[14ch] text-[clamp(2.8rem,6.2vw,5.2rem)] font-[760] leading-[0.94] tracking-[-0.06em]">
              Find meaningful gene interactions.
            </h1>
          </div>
          <p className="max-w-xl text-[17px] font-medium leading-7 text-muted">
        ...
        <section aria-labelledby="method-heading" className="mt-12">
          ...
          <div className="grid overflow-hidden rounded-[16px] border border-line bg-line shadow-soft md:grid-cols-2 xl:grid-cols-4">
            {STEPS.map((step) => (
              <article key={step.number} className="flex min-h-72 flex-col bg-paper p-5 sm:p-6">
                <p className="font-mono text-[10px] font-medium tracking-[0.12em] text-muted">{step.number}</p>
                <div className="mt-7 flex min-h-16 items-center">{step.visual}</div>
                <h3 className="mt-7 text-lg font-[700] tracking-[-0.025em]">{step.title}</h3>
        ...
        <section className="mt-10 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <article className="surface-card p-6 sm:p-7">
            <h2 className="text-xl font-[720] tracking-[-0.03em]">Three views of the same cohort</h2>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-muted">
              CBaSE is the primary background model. DIG and MutSigCV2 are sensitivity analyses. The default keeps the exact pair only when the same direction is significant under all three backgrounds.
            </p>
            <p className="mt-4 border-l-2 border-brand pl-4 text-sm font-medium leading-6 text-ink">
              This is same-cohort sensitivity agreement—not independent replication. “Significant under all three” means q &lt; 0.01 separately in CBaSE, DIG, and MutSigCV2; MutSig features that reuse a CBaSE background are excluded.
            </p>
          </article>
          <article className="rounded-[14px] border border-co/20 bg-co-soft/60 p-6 sm:p-7">
            <p className="font-mono text-[10px] font-medium uppercase tracking-[0.12em] text-co">CO caution</p>
            <h2 className="mt-3 text-xl font-[720] tracking-[-0.03em]">Tumor burden can mimic co-occurrence.</h2>
        ...
        <section aria-labelledby="terms-heading" className="mt-12">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <h2 id="terms-heading" className="text-2xl font-[740] tracking-[-0.035em]">Read the notation</h2>
            <Tooltip>
              <TooltipTrigger asChild>
                <button type="button" className="focus-ring rounded-[8px] text-xs font-semibold text-muted underline decoration-line underline-offset-4 hover:text-ink">
          ...
          <dl className="mt-5 grid gap-px overflow-hidden rounded-[14px] border border-line bg-line sm:grid-cols-2 lg:grid-cols-3">
            {GLOSSARY.map((item) => (
              <div key={item.term} className="bg-paper p-5">
                <dt className="font-mono text-sm font-medium text-ink">{item.term}</dt>
                <dd className="mt-2 text-xs leading-5 text-muted">{item.definition}</dd>
              </div>
            ))}
          </dl>
        </section>
        ...
        <section aria-labelledby="release-heading" className="surface-card mt-12 p-6 sm:p-7">
          <div className="flex flex-wrap items-start justify-between gap-5">
            <div>
              <div className="inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.1em] text-support">
                <LockKeyhole className="size-3.5" aria-hidden />
                {bundle.manifest.immutable ? "Immutable release" : "Release"}
              </div>
              <h2 id="release-heading" className="mt-2 text-xl font-[720] tracking-[-0.025em]">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="focus-ring inline-flex items-center gap-1.5 rounded-[8px] text-xs font-semibold text-muted underline decoration-line underline-offset-4 hover:text-ink"
            >
              Source code <ExternalLink className="size-3.5" aria-hidden />
            </a>
          </div>

          <dl className="mt-6 grid gap-px overflow-hidden rounded-[12px] border border-line bg-line sm:grid-cols-2 lg:grid-cols-4">
